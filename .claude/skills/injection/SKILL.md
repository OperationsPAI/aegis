...
- **Stress params: check the target pod's resources first, then tune for LATENCY degradation — not OOM-kill.** Before a `cpu_stress`/`memory_stress` inject, look up the target pod's limits and live usage (`kubectl get deploy <svc> -o jsonpath='{.spec.template.spec.containers[0].resources}'` + `kubectl top pod`; or fall back to the post-#563 chart values — light Go/C++ services ≈ `cpu 500m / mem 1Gi`, JVM services `cpu 3-5 / mem 3-4Gi`). The goal is a detectable latency/SLO breach, **not** killing the pod — a hard OOM-kill just restarts it (cruder, and risks an empty build window).
  - **cpu_stress**: set `cpu_worker × cpu_load` so the stress demand *plus the service's own CPU* clears the CPU **limit** → CFS throttling → request latency climbs. Post-#563 limits are ~500m, so a few workers at 80–90% clear them; for ts's 5-core limits you need more workers or a heavier-loaded on-path service.
  - **memory_stress** (StressChaos) is a *sized cgroup allocation* — its bite depends on `memory_size` vs `(limit − current usage)`, NOT on the runtime. Size it to genuinely pressure the cgroup (approach the limit) so latency moves; too small is just absorbed (otel-demo 600Mi at a 1Gi limit → no_anomaly), so don't lowball it — but don't overshoot into a hard OOM-kill either (restarts the pod, empty-window risk).
  - **jvm_memory_stress** (JVMChaos, JVM services only — ts/teastore/sockshop) is the *direct* path and a DIFFERENT chaos type: it drives the JVM agent to exhaust the heap and trip the OOM-killer with NO `memory_size` calibration, independent of how much memory is actually in use. Reach for it when you want a reliable JVM memory fault without sizing against the cgroup. (Don't confuse the two — the size-independent "just OOMs" behavior belongs to jvm_memory_stress, not memory_stress.)
