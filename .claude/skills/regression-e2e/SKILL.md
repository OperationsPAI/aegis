...
- **Empty required parquet** — BuildDatapack is fail-fast in production; there is no env-var bypass. Run `aegisctl datapack diagnose --injection <id>` to identify which parquet and why. The usual root cause is an inject against a service with no traffic — fix the targeting or pre-screen with `aegisctl reason filter-clean`. `python cli/prepare_inputs.py run --allow-empty` exists for local/offline investigation only and is deliberately not part of the in-cluster Job entrypoint.
