...
- **JVMChaos `exit status 101`** — chaos-mesh's JVMChaos hardcodes `sh -c "mkdir -p /usr/local/byteman/lib/"` inside the target pod's mount namespace. Distroless / busybox-only images (e.g. `distroless/java*:debug` puts shell at `/busybox/sh`, not `/bin/sh`) silently fail this step before bminstall ever runs. Switch the workload's base image to one with `/bin/sh` (eclipse-temurin / openjdk-slim / alpine variants).
- **CRD events silently dropped: "Ignoring CRD add event for inactive namespace X"** — runtime-worker's chaos-resource controller maintains an active-namespaces filter; once it marks an ns inactive (via system disable / count shrink), CRDs in that ns get dropped even though chaos-mesh successfully created them. Symptom: trace stays `last_event: restart.pedestal.completed` indefinitely; `kubectl -n <ns> get jvmchaos` shows the CRD exists. Restart of the worker pod alone may NOT recover (active-set has a persistent source). After the lock-store reactivate hook is in place, every successful AcquireLock re-marks the ns active automatically; if you still see this, file an issue with the worker log lines.
- **Trace stuck at `restart.pedestal.completed` waiting on prior orphan lock** — a previously-failed trace's CRD can hold the redis ns lock until its duration window naturally elapses. Symptom: worker log shows `failed to acquire lock for required namespace X: namespace X is locked by <other-trace-id> until <timestamp>`. Either wait for the lock to expire, or cancel the orphan trace via `aegisctl trace cancel <trace-id> -f` to release immediately.
- **`exit status 101` not chaos-mesh-shell-related** — same error code can mean JVM attach failures unrelated to image: JDK 16+ early-access on Alpine/musl, `-XX:+DisableAttachMechanism`, or container security context blocking `/proc/<pid>/root` access. Inspect chaos-daemon log on the target pod's node — the actual subprocess stderr is hidden by the gRPC `Unknown` wrapper (chaos-mesh#4256).
