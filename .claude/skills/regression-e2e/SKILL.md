...
- **Duplicate-submission suppression** — aegisctl regression silently refuses re-submitting the same (app, namespace, chaos_type, duration) within a cooldown; symptom is `duplicate submission suppressed (batches [0])`. Vary `spec[].app` or `duration` to force a fresh hash. Note `duration` defaults to 5 minutes when omitted, so two submissions both omitting it collide on the same hash — pass an explicit value to disambiguate.
