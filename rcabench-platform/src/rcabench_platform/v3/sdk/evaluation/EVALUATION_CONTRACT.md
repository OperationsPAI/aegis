...
| 文件 | 层 | 职责 |
|---|---|---|
| `agent_output.py` | 契约 | Agent 输出 `AgentRCAOutput`：`root_causes` + `propagation`，每条 `RootCauseClaim` 至少 1 条 evidence |
| `agent_contract.py` | 契约 | `get_agent_contract_prompt()` / `get_fault_kind_disambiguation()`：拼装下发给 agent 的 schema + 枚举 + 辨析文档 |
| `fault_kind_disambiguation.md` | 契约 | 每个 fault_kind 一句话语义 + 跨等价类边界规则 + 等价类表（pod / 网络阻断）|
| `fault_kind.py` | 词表 | 26 类故障枚举（SRE 词汇）+ `chaos_type` 映射 + `_LEGACY_ALIASES`（老字符串仍可 parse） |
| `ground_truth.py` | GT | 从 `injection.json` 提取 `GTFault` 列表 |
| `ranking.py` | L1 | 传统算法的 Top-k / MRR / runtime 聚合（dataframe→ AC@k） |
| `set_match.py` | L2 | `(service, fault_kind)` 多重集匹配 → **strict** + **service-only** 两套 P/R/F1 + `fault_kind_accuracy` + 等价类合并 |
| `graph_match.py` | L3 | 服务级 `node_f1` / `edge_f1` + `path_reachability`（HIT 根因→GT alarm 服务在 agent propagation 上是否可达） |
| `sql_verify.py` | L4a | DuckDB 在 case 的 parquet 目录上 re-run 每条 evidence SQL（OK / EMPTY / SQL_ERROR） |
| `chain_judge.py` | L4b | LLM-as-judge：**逐 evidence** 判定 "SQL 行集合是否支撑 claim 且整条链不断裂" |
| `evaluator.py` | 入口 | `evaluate()` 串起以上层，组合 `EvaluationResult` |
| `result.py` | schema | `EvaluationResult` 平铺字段 schema |
...
injection.json ─→ list[GTFault]         (无 GT → parse_error)
                ↓
   set_match.compute_outcome            (匹配 + strict & service-only P/R/F1 + exact + kind_accuracy)
   graph_match.compute_graph_metrics    (服务级 node_f1 / edge_f1)
   graph_match.compute_path_reachability (HIT→alarm 连通性)
...
| `f1`        | 二者调和平均 | 综合定位质量 |
| `service_precision` / `service_recall` / `service_f1` / `service_exact_match` | 把 HIT 和 WRONG_KIND 都当作"服务对了"重新算 P/R/F1/EM。`service_*` ≥ 对应的 strict 同名指标 | 只看「服务定位」，不看 fault_kind 是否对 |
...
- 单 case 完整打分：`sample.eval_metrics`（`EvaluationResult.model_dump`），含 `per_fault`、`overclaim_indices`、`per_evidence`、`graph_metrics`、各 evidence 的 judge 输出。历史行写在 `sample.meta["eval_v2"]`，processer 会 fallback 读取。
