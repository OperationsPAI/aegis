...
| `evaluator.py` | 串起以上五步，组合 `EvaluationResult` |
...
- 单 case 完整打分：`sample.meta["eval_v2"]`（`EvaluationResult.model_dump`），含 `per_fault`、`overclaim_indices`、`per_evidence`、`propagation_metrics`、各 evidence 的 judge 输出。
