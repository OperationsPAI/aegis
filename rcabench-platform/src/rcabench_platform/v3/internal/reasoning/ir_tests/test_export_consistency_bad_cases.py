...
BETA_DATASET_CASES = Path("/home/ddq/AoyangSpace/dataset/dataset_v3_beta_500_2026-05-04/cases")
...
def _beta_case_json(case_name: str) -> tuple[dict, dict]:
    case_dir = BETA_DATASET_CASES / case_name
    return (
        json.loads((case_dir / "causal_graph.json").read_text()),
        json.loads((case_dir / "result.json").read_text()),
    )
...
def test_alarm_normalization_regression_bare_endpoint_strong_match() -> None:
    causal_graph, _result = _beta_case_json("hs4-recommendation-pod-failure-lq9fmq")
    matches = [
        node
        for node in causal_graph["candidate_alarm_nodes"]
        if node["component"] == "span|frontend::HTTP /recommendations"
    ]
    assert matches
    detail = matches[0]
    assert detail["issue_strength"] == "strong"
    assert detail["conclusion_span_name"] == "HTTP /recommendations"
    assert detail["conclusion_match"]["status"] == "matched"
    assert detail["conclusion_match"]["method"] == "bare_operation_unique"
    assert causal_graph["candidate_strong_alarm_count"] > 0
