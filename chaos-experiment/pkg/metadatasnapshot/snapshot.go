// Package metadatasnapshot exposes a read-only view of internal/* metadata
// for the migrate-chaos-metadata dump tool.
//
...
// SpanNames is the set of distinct OTel span names observed on the
// source→target edge; consumed by groundtruth to label affected spans.
...
func DumpForSystem(system string) (Snapshot, error) {
	...
	for i, np := range net {
		snap.NetworkPairs[i] = NetworkPair{
			...
			SpanNames:     append([]string(nil), np.SpanNames...),
		}
	}
