...
**Workaround attempted**: reran with `--retain -v 1` and inspected `docker logs` to identify the real cause; attempted `sysctl -w fs.inotify.max_user_instances=1024` and `sysctl -w fs.inotify.max_user_watches=524288`, but non-root writes were denied
