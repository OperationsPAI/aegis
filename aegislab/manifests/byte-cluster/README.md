...
The admin password is generated by SSO on first boot and dumped into the
SSO PVC at `/var/lib/sso/.first-boot-secret.admin` — see
[`../AUTHENTICATION.md`](../AUTHENTICATION.md) for the full flow.
Retrieve it once, then exchange for a JWT:
...
cd aegislab
echo "$ADMIN_PW" | ./bin/aegisctl auth login \
  --server http://127.0.0.1:8082 \
  --username admin --password-stdin

./bin/aegisctl --server http://127.0.0.1:8082 system list -o json
cd ..
```

The OIDC backend client secret is mirrored into the `sso-bootstrap`
K8s Secret (`aegis-backend-secret` key) for downstream services that
need it without mounting the SSO PVC.
