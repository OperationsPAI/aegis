...
func loadGuidedConfigFile(path string) (*guidedcli.ConfigFile, error) {
	if path == "" {
		var err error
	...
	if err != nil {
		if os.IsNotExist(err) {
			return &guidedcli.ConfigFile{
				Version:        1,
				CurrentContext: "default",
				Contexts:       map[string]guidedcli.CLIContext{"default": {}},
			}, nil
		}
		return nil, fmt.Errorf("read config file: %w", err)
	}

	var cfg guidedcli.ConfigFile
	...
	if cfg.Contexts == nil {
		cfg.Contexts = map[string]guidedcli.CLIContext{"default": {}}
	}
...
func saveGuidedConfigFile(path string, cfg *guidedcli.ConfigFile, snapshot guidedcli.GuidedConfig) error {
	if path == "" {
		var err error
		path, err = defaultGuidedConfigPath()
		if err != nil {
	...
	if cfg == nil {
		cfg = &guidedcli.ConfigFile{Version: 1}
	}
	if cfg.Version == 0 {
		cfg.Version = 1
	}
	cfg.GuidedSession = guidedcli.GuidedSession{
		Config:    snapshot,
...
func mergeGuidedConfig(fileCfg *guidedcli.ConfigFile, cliCfg guidedcli.GuidedConfig) guidedcli.GuidedConfig {
	merged := guidedcli.GuidedConfig{}
	if fileCfg != nil {
...
func overlayGuidedConfig(dst *guidedcli.GuidedConfig, src guidedcli.GuidedConfig) {
	if src.System != "" || src.SystemType != "" || src.Namespace != "" {
		duration := dst.Duration
		*dst = guidedcli.GuidedConfig{}
...
func clearGuidedFromChaosType(cfg *guidedcli.GuidedConfig) {
	cfg.ChaosType = ""
	cfg.Container = ""
	cfg.TargetService = ""
	cfg.Domain = ""
