...
### Hard rules for new HTTP endpoints

Three things every new endpoint MUST do — no exceptions, no "I'll do it later":

1. **Swagger annotations on the handler** — `@Router`, `@Param`, `@Success`, `@Failure`, `@Tags`, `@Security`, and `@x-api-type {"sdk":"true"}` if external clients consume it. The OpenAPI spec is generated from these comments; missing them = missing SDK methods = downstream consumers reach for hand-rolled HTTP.

2. **Regenerate the SDK in the same commit** — `cd aegislab && just generate-go-sdk 0.0.0` (and `generate-portal` / `generate-admin` / `generate-python-sdk` if the audience matters). The `swagger freshness` CI gate fails the PR otherwise. **Never** hand-roll a request to your own API in `aegisctl` or `frontend` — use the generated `apiclient.<Service>.<Method>` from `src/cli/apiclient/`. If the SDK method doesn't exist, the annotation is missing — go back to step 1.

3. **Federate through `rcabench-gateway`** if the endpoint lives on a service other than `aegis-api` (`aegis-chaos`, `aegis-sso`, `aegis-blob`, etc.). Add one `[[gateway.routes]]` entry to both `src/config.dev.toml` and `helm/templates/configmap.yaml` (mirror the existing `chaos` entry); widen the `@api` matcher in `helm/templates/edge-proxy-config.yaml` if the path prefix is new. External clients reach every service through the same EIP/JWT pipeline — no separate `--<service>-server` flags, no `kubectl port-forward` shortcuts.

Violations of any of these surface as: ① CI fails (swagger freshness), ② downstream PRs reinvent transport code, ③ ops have to PF every service individually. All three are fixable but unnecessary if the original PR follows the rules.
...
2. Regenerate the SDK(s) — all targets share the same `swag init` step internally, so you can run any of:
```bash
just generate-portal      0.0.0   # TypeScript portal
just generate-admin       0.0.0   # TypeScript admin
just generate-python-sdk  0.0.0   # Python (sdk/python)
just generate-go-sdk      0.0.0   # Go (src/cli/apiclient — used by aegisctl)
```
...
### SDK Files Location
- **TypeScript SDK**: `sdk/typescript/{portal,admin}/` — used by frontend via `@OperationsPAI/portal`
- **Python SDK**: `sdk/python/` — used by external Python clients
- **Go SDK**: `src/cli/apiclient/` — typed client + DTOs consumed by aegisctl (lives inside the `aegis` module so cobra commands can import it directly; the standalone `go.mod` the generator emits is stripped during install)

### API Filtering Logic
All SDK targets are driven from the same OpenAPI 3 spec at `src/docs/openapi3/openapi.json`, generated by `swag init` from handler comments. The unified pipeline lives at `scripts/command/src/swagger/` (`init.py` regenerates the spec, then `python.py` / `typescript.py` / `golang.py` each emit one target). All targets are filtered by the `x-api-type.sdk` audience flag — only handlers annotated with `@x-api-type {"sdk":"true"}` ship to external SDKs.
...
- **SDK 同步**: API 变更后必须 `just generate-go-sdk 0.0.0` (+ portal/admin/python 视消费方而定); aegisctl/frontend **禁止手写 HTTP 请求**,只能用 `apiclient.<Service>.<Method>` —— 见上方 "Hard rules for new HTTP endpoints"
- **新端点入网关**: 非 aegis-api 的服务 (chaos/sso/blob/...) 必须通过 `rcabench-gateway` federate,不允许单独 EIP / 必须 PF
