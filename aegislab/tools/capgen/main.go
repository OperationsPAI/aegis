...
func writeGoMap(path string, caps []capability) error {
	pairs := make([][2]string, len(caps))
	for i, c := range caps {
		pairs[i] = [2]string{c.ChaosType, c.Name}
	}
	sort.Slice(pairs, func(i, j int) bool { return pairs[i][0] < pairs[j][0] })

	maxKeyLen := 0
	for _, p := range pairs {
		if n := len(p[0]); n > maxKeyLen {
			maxKeyLen = n
		}
	}

	var sb strings.Builder
	sb.WriteString("// Code generated by capgen. DO NOT EDIT.\n")
	sb.WriteString("// Source: aegislab/tools/capgen (chaos_type -> capability name).\n")
	sb.WriteString("// Regenerate with `just capgen` from aegislab/.\n\n")
	sb.WriteString("package chaos\n\n")
	sb.WriteString("// ChaosTypeToCapability maps the upstream chaos-experiment ChaosType\n")
	sb.WriteString("// enum to the canonical capability name. Lives here (not under cli/)\n")
	sb.WriteString("// so both aegisctl and the backend pre-flight catalog validator can\n")
	sb.WriteString("// address the same Point identity.\n")
	sb.WriteString("var ChaosTypeToCapability = map[string]string{\n")
	for _, p := range pairs {
		padding := strings.Repeat(" ", maxKeyLen-len(p[0]))
...
func main() {
	goOut := flag.String("go-out", "../../src/crud/chaos/capability_map_gen.go",
		"path to write the generated Go ChaosTypeToCapability map; empty disables Go emission")
	flag.Parse()
	...
	if *goOut != "" {
		if err := writeGoMap(*goOut, caps); err != nil {
			fmt.Fprintln(os.Stderr, "capability_map_gen.go:", err)
			os.Exit(1)
		}
	}
