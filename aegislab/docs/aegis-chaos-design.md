...
| Dedup behavior | Idempotency key required (`--idempotency-key`); duplicate POSTs return the existing injection row, never re-invoking `Executor.Apply` (see §12.3, invariants I2/I7) | Backend-side dedup keyed on point/params/window (see §12.3, invariants I2/I7); suppressed submits surface a dedupe-suppressed sentinel |
